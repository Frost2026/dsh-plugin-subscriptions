/**
 * The pool adapter: family aggregation, member selection (priority failover
 * and quota-aware urgency scheduling with sticky hysteresis), stream failover
 * semantics (switch before the first chunk, never after), and capability
 * intersection for pooled models. All members are fake in-memory adapters.
 */
...
import type { PoolMemberRef } from '../src/providers/pool-family.js'
...
/** A scripted member adapter: serves chunks from `serve`, counts calls. */
class FakeAdapter extends LlmAdapter {
  calls = 0

  constructor(
    private readonly serve: (options: GenerateOptions) => AsyncIterable<StreamChunk>,
...
const freshFamily = (): Map<string, PoolMemberRef[]> =>
  new Map([['fam', [{ provider: 'codex', model: 'm' }, { provider: 'claude', model: 'm' }]]])
...
function makePool(
  ...
  const usage = new PoolUsageTracker(options.usage ?? {})
...
test('buildFamilyPools aggregates across providers, copilot last', () => {
  const catalog = (provider: ProviderId, ids: string[]): LlmModelInfo[] =>
    ids.map(id => ({ provider, id, name: id }))
  const pools = buildFamilyPools({
    copilot: catalog('copilot', ['gpt-5.4', 'claude-sonnet-4.5', 'gemini-2.5-pro']),
    codex: catalog('codex', ['gpt-5.4', 'gpt-5.4-mini']),
    claude: catalog('claude', ['claude-sonnet-4-5-20250929']),
    grok: catalog('grok', ['grok-4.6']),
  })
  ...
  // Single-provider families (grok, gemini, gpt-5.4-mini) never become pools.
...
test('priority: the first healthy member serves', async () => {
...
test('priority: a cooling member is skipped', async () => {
  ...
  health.markUnavailable(memberKey('codex', 'm'), 60_000, 'QUOTA')
  const chunks = await collect(pool.stream(OPTIONS))
...
test('quota_aware: a member without telemetry (copilot) sinks to the bottom', async () => {
  ...
  const family = new Map<string, PoolMemberRef[]>([
    ['fam', [{ provider: 'codex', model: 'm' }, { provider: 'copilot', model: 'm' }]],
  ])
...
test('stream: a pre-chunk quota failure cools the whole provider and fails over', async () => {
  ...
  await usage.quotaFor({ provider: 'codex', model: 'm' })
  assert.equal(usageCalls.length, 1)
  const chunks = await collect(pool.stream(OPTIONS))
  assert.equal((chunks[0] as { text: string }).text, 'claude')
  assert.equal(codex.calls, 1)
  assert.equal(claude.calls, 1)
  assert.equal(health.isMemberAvailable('codex', 'm'), false)
  // Codex quota is account-level: sibling models are parked too.
  assert.equal(health.isMemberAvailable('codex', 'other-model'), false)
...
test('stream: a claude quota failure cools only the failing member', async () => {
  ...
  const codex = new FakeAdapter(() => serveOk('codex'))
  const family = new Map<string, PoolMemberRef[]>([
    ['fam', [{ provider: 'claude', model: 'claude-opus-5' }, { provider: 'codex', model: 'm' }]],
  ])
  const { pool, health } = makePool({ claude, codex }, { families: family })
  await collect(pool.stream(OPTIONS))
  assert.equal(health.isMemberAvailable('claude', 'claude-opus-5'), false)
  // Claude quota windows are model-scoped: the sibling lane stays available.
  assert.equal(health.isMemberAvailable('claude', 'claude-sonnet-5'), true)
})
...
test('stream: request-fault failures rethrow without trying other members', async () => {
  ...
  assert.equal(health.isAvailable(memberKey('codex', 'm')), true)
})
