 ...
 * xAI rejects a request carrying `tool_choice` without `tools` with 400
 * invalid-argument, which broke every tool-less `ctx.llm.stream` call
 * (plugins judging or verifying with a bare prompt) while the agent loop —
 * which always sends tools — kept working.
 */
